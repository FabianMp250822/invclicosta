...
const GeneralInfo = ({ researcher }) => {
  ...
  const { biografia, informacion_personal, redes_sociales_academicas, identificadores_de_autor } = researcher;

  return (
    <div style={{ padding: '1.5rem' }}>
      <h5 style={{ backgroundColor: '#f0f0f0', padding: '0.8rem', borderRadius: '8px' }}>General Information</h5>

      {biografia && biografia.texto && (
        <div style={{ marginBottom: '1.5rem', padding: '0.75rem', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0px 1px 3px rgba(0, 0, 0, 0.2)' }}>
      ...
      <div style={{ marginBottom: '1.5rem', padding: '0.75rem', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0px 1px 3px rgba(0, 0, 0, 0.2)' }}>
        ...
        <p><strong>Citation Name:</strong> {informacion_personal?.nombre_en_citaciones || 'No data'}</p>
        <p><strong>Sex:</strong> {informacion_personal?.sexo || 'No data'}</p>
        <p><strong>Nationality:</strong> {informacion_personal?.nacionalidad || 'No data'}</p>
        <p><strong>Place of Birth:</strong> {informacion_personal?.lugar_nacimiento || 'No data'}</p>
      </div>

      <div style={{ marginBottom: '1.5rem', padding: '0.75rem', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0px 1px 3px rgba(0, 0, 0, 0.2)' }}>
        <h6 style={{ marginBottom: '0.8rem' }}>Academic Social Networks and Identifiers</h6>
        <div style={{ display: 'flex', flexWrap: 'wrap' }}>
          {Object.entries(redes_sociales_academicas || {}).map(([key, value]) => {
            const IconComponent = iconMapping[key] || getIcon(value);
            return (
              <div key={key} style={{ width: '45px', height: '45px', margin: '0.4rem', backgroundColor: '#f0f0f0', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          ...
          {identificadores_de_autor?.autor_id_scopus && (
            <div style={{ width: '45px', height: '45px', margin: '0.4rem', backgroundColor: '#f0f0f0', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
